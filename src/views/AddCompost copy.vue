<template>
  <div id="addcompost-container">
    <v-card class="mx-auto my-5" max-width="344">
      ...
      <v-card-text>
        <v-container>
          <div class="form-container mt-24 container mx-auto">
            <div class="relative card p-4 bg-white">
              <DynamicForm :form="form" @submitted="submitForm">
                <template
                  v-slot:address="{ control, onChange, onFocus, onSubmit }"
                >
                  <input
                    type="input"
                    :id="control.name"
              ...
              <div class="error-msg" v-if="error.msg">{{ error.msg }}</div>
            </div>
          </div>
        </v-container>
      </v-card-text>
